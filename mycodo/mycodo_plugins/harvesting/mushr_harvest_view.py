...
WIDGET_INFORMATION = {
    'widget_name_unique': 'mushr_harvest_view',
    'widget_name': 'MushR Harvest View Widget',
    'widget_library': '',
    'no_class': True,
    ...
    'message': 'This widget views Harvest history for MushR',
...
<script type="text/javascript">
  ...
  function draw_harvest(datetime) {
      posix_epoch = datetime.getTime()/1000;

      var initial_cypher = "MATCH (mh)-[r1:IS_HARVESTED_FROM]->(fl)-[r2:FRUITS_FROM]->(s) WHERE mh.dateHarvested <= "+posix_epoch+" RETURN mh,fl,s,r1,r2";

      var update_cyphers_harvest = [

    "MATCH (fl)-[r1:FRUITS_THROUGH]->(fh)-[r3:IS_PART_OF]->(subc) WHERE r1.start <= "+posix_epoch+" AND (not exists(r1.end) or r1.end >= "+posix_epoch+") WITH fl, fh, subc, r1, r3 MATCH (fl)-[r2:FRUITS_FROM]->(sub:Substrate)-[r4:IS_CONTAINED_BY]->(subc) RETURN sub, fl, fh, subc, r1, r2, r3, r4", // Get currently fruiting flushes

    "MATCH (subc)-[r1:IS_LOCATED_AT]->(gc:GrowChamber) WHERE r1.start <= "+posix_epoch+" AND (NOT exists(r1.end) or r1.end >= "+posix_epoch+") WITH subc, gc, max(r1.start) as maxr1start MATCH (subc)-[r1:IS_LOCATED_AT]->(gc) WHERE r1.start = maxr1start RETURN subc, r1, gc" // Get Location of SubstrateContainers (if they are in a GrowChamber)
	...
	return title
      ...
      viz_harvest = new NeoVis.default(config);
      viz_harvest.render();
    for(var i = 0; i<update_cyphers_harvest.length;i++)
    {
    console.log(update_cyphers_harvest[i]);
    viz_harvest.updateWithCypher(update_cyphers_harvest[i]);
    }
  }
  $("document").ready(function(){
      draw_harvest(new Date())
  });
</script>

<div id="viz_harvest"> </div>
<div class="input-group">
  <input id="datetime_viz_harvest" type="datetime-local" class="form-control" placeholder="Datetime" aria-label="Visualization DateTime" aria-describedby="basic-addon2">
  <div class="input-group-append">
    <button id="datetime_viz_harvest_button" class="btn btn-outline-secondary" type="button">R</button>
  </div>
</div>

<script type="text/javascript">
  $("#datetime_viz_harvest_button").click(function(){
      draw_harvest(new Date($("#datetime_viz_harvest").val()))
  });
