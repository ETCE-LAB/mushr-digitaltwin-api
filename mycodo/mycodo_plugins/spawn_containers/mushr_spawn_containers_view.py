...
WIDGET_INFORMATION = {
    'widget_name_unique': 'mushr_spawn_container_view',
    'widget_name': 'MushR SpawnContainer View Widget',
    'widget_library': '',
    'no_class': True,
    ...
    'message': 'This widget views SpawnContainers for MushR',
...
<script type="text/javascript">
  var viz_spawn_container;

  function draw_spawn_container(datetime) {
      posix_epoch = datetime.getTime()/1000;

      var initial_cypher = "MATCH (n:SpawnContainer) WHERE n.dateCreated <= "+posix_epoch+" RETURN n";

      var update_cyphers_spawn_containers = [

    "MATCH (n:SpawnContainer)-[R:IS_LOCATED_AT]->() WHERE n.dateCreated <= "+posix_epoch+" AND R.start <= "+posix_epoch+" WITH n, max(R.start) as maxstart MATCH (n)-[r]->(n2) where r.start=maxstart return r,n,n2" // Get location of SpawnContainers
	...
	return title
      ...
      var config = {
	  containerId: "viz_spawn_container",
	  neo4j: {
      ...
      viz_spawn_container = new NeoVis.default(config);
      viz_spawn_container.render();
      for(var i = 0; i < update_cyphers_spawn_containers.length; i++){
            console.log(update_cyphers_spawn_containers[i]);
            viz_spawn_container.updateWithCypher(update_cyphers_spawn_containers[i]);
      }
  }
  $("document").ready(function(){
      draw_spawn_container(new Date())
  });
</script>
<div id="viz_spawn_container"> </div>
<div class="input-group">
  <input id="datetime_viz_spawn_container" type="datetime-local" class="form-control" placeholder="Datetime" aria-label="Visualization DateTime" aria-describedby="basic-addon2">
  <div class="input-group-append">
    <button id="datetime_viz_spawn_container_button" class="btn btn-outline-secondary" type="button">R</button>
  </div>
</div>

<script type="text/javascript">
  $("#datetime_viz_spawn_container_button").click(function(){
      draw_spawn_container(new Date($("#datetime_viz_spawn_container").val()))
  });
