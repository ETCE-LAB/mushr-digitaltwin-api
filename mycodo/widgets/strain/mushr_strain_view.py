...
WIDGET_INFORMATION = {
    'widget_name_unique': 'mushr_strain_view',
    'widget_name': 'MushR Strain View Widget',
    'widget_library': '',
    'no_class': True,
    ...
    'message': 'This widget views Strains for MushR',
...
<script type="text/javascript">
  ...
  function draw_strain(datetime) {
      posix_epoch = datetime.getTime()/1000;
      var initial_cypher = "MATCH (n:Strain) WHERE n.dateCreated <= "+posix_epoch+" return n";
      var update_cyphers_strain = [
    ];
	...
	return title
      ...
      viz_strain = new NeoVis.default(config);
      viz_strain.render();
    for(var i = 0; i<update_cyphers_strain.length;i++)
    {
    console.log(update_cyphers_strain[i]);
    viz_strain.updateWithCypher(update_cyphers_strain[i]);
    }
  }
  $("document").ready(function(){
      draw_strain(new Date())
  });
</script>

<div id="viz_strain"> </div>
<div class="input-group">
  <input id="datetime_viz_strain" type="datetime-local" class="form-control" placeholder="Datetime" aria-label="Visualization DateTime" aria-describedby="basic-addon2">
  <div class="input-group-append">
    <button id="datetime_viz_strain_button" class="btn btn-outline-secondary" type="button">R</button>
  </div>
</div>

<script type="text/javascript">
  $("#datetime_viz_strain_button").click(function(){
      draw_strain(new Date($("#datetime_viz_strain").val()))
  });
