...
WIDGET_INFORMATION = {
    'widget_name_unique': 'mushr_storage_view',
    'widget_name': 'MushR StorageLocation View Widget',
    'widget_library': '',
    'no_class': True,
    ...
    'message': 'This widget views StorageLocations for MushR',
    ...
    'widget_dashboard_head': """<!-- Required scripts -->
...
<script type="text/javascript">
  ...
  function draw_storage(datetime) {
      posix_epoch = datetime.getTime()/1000;
      var initial_cypher = "MATCH (n) WHERE (n:Location OR n:GrowChamber) AND n.dateCreated <= "+posix_epoch+" RETURN n";
      var update_cyphers_storage = [
    "MATCH (n)-[R:IS_LOCATED_AT]->(n2) WHERE n.dateCreated <= "+posix_epoch+" AND R.start <= "+posix_epoch+" and (NOT exists(R.end) OR R.end >= "+posix_epoch+") WITH n, n2, max(R.start) as maxstart MATCH (n)-[r]->(n2) where r.start=maxstart return r,n"
    ];
	...
	return title
      ...
      viz_storage.render();
    for(var i = 0; i<update_cyphers_storage.length;i++)
    {
    console.log(update_cyphers_storage[i]);
    viz_storage.updateWithCypher(update_cyphers_storage[i]);
    }
  }
  $("document").ready(function(){
      draw_storage(new Date())
  });
</script>
<div id="viz_storage"> </div>
<div class="input-group">
  <input id="datetime_viz_storage" type="datetime-local" class="form-control" placeholder="Datetime" aria-label="Visualization DateTime" aria-describedby="basic-addon2">
  <div class="input-group-append">
    <button id="datetime_viz_storage_button" class="btn btn-outline-secondary" type="button">R</button>
  </div>
</div>

<script type="text/javascript">
  $("#datetime_viz_storage_button").click(function(){
      draw_storage(new Date($("#datetime_viz_storage").val()))
  });
